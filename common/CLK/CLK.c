...
extern UINT8 set_update_phy_flag[];
...
//		}
		...
		{
			if((xds.xfer_Nmax > (120/* 120 */))&&(!xds.xfer_first)) //exit fast if regression is sufficiently stabilized Jan 2010
			{
				tstate = FLL_NORMAL;
			}
		}
		...
		{
			if((xds.xfer_Nmax > (100/* 100 */))&&(!xds.xfer_first)) //exit fast if regression is sufficiently stabilized Jan 2010
			{
				tstate = FLL_NORMAL;
			}
		}
...
static void update_time(void)
{
   ...
   INT32 avg_TIME;
   UINT8 update_phy_flag;
#ifdef MODIFY_BY_JIWANG
   static UINT8 set_24us_flag = 0;
#endif
   ...
   case RM_GM_1:
	...
	case RM_GPS_1:
      avg_TIME=avg_GPS_A;
#ifdef MODIFY_BY_JIWANG
	  update_phy_flag = set_update_phy_flag[GPS_A];
#endif
      break;
	case RM_GPS_2:
      avg_TIME=avg_GPS_B;
#ifdef MODIFY_BY_JIWANG
	  update_phy_flag = set_update_phy_flag[GPS_B];
#endif
      break;
	case RM_GPS_3:
      avg_TIME=avg_GPS_C;
#ifdef MODIFY_BY_JIWANG
	  update_phy_flag = set_update_phy_flag[GPS_C];
#endif
      break;
	case RM_GPS_4:
      avg_TIME=avg_GPS_D;
#ifdef MODIFY_BY_JIWANG
	  update_phy_flag = set_update_phy_flag[GPS_D];
#endif
      break;
	case RM_SE_1:
      avg_TIME=avg_GPS_A;
...
//				#if(GPS_ENABLE==1)
#ifdef MODIFY_BY_JIWANG
			if(FLL_Elapse_Sec_Count_Use > 600){
				time_hold_off = 4;
			}
...
//         if(1)  //TEST TEST TEST
         #if (LOW_PACKET_RATE_ENABLE == 1)
#ifdef MODIFY_BY_JIWANG
			if((tstate!=FLL_WARMUP)&&(FLL_Elapse_Sec_Count_Use<4000) && (time_transient_flag < 2) && !update_phy_flag)  //was 5200
#else
...
#ifdef MODIFY_BY_JIWANG
			else if(((fll_settle==0) && (time_transient_flag==0)) || (update_phy_flag && (time_transient_flag==0)))
			{
				printf("================>>update_phy_flag = %hhu\n", update_phy_flag);
#else
...
#ifdef MODIFY_BY_JIWANG
			if(update_phy_flag){
				if(abs(time_slew_err) > 1000){
					ltgain = 0.2;
				}
				else if(abs(time_slew_err) >100 && abs(time_slew_err) <=1000)
					ltgain = 0.1;
				else
					ltgain = 0.01;
			}
...
int Get_servoAccept(t_servoChanAcceptType *p_servoChanAccept)
{
   ...
   double foffset;
#ifdef MODIFY_BY_JIWANG
   static UINT32 clr_phy_update_cnt[7] = {0,0,0,0,0,0,0};
#endif
#if 1
...
//         (int)Phy_Channel_Transient_Free_Cnt[chan-RM_GPS_1]);
	  ...
	  if(set_update_phy_flag[chan-RM_GPS_1]){
		  clr_phy_update_cnt[chan-RM_GPS_1] = 600 - 30;
	  }	
	  else
		  if(clr_phy_update_cnt[chan-RM_GPS_1] > 0)
				clr_phy_update_cnt[chan-RM_GPS_1] --;

	  if(clr_phy_update_cnt[chan-RM_GPS_1] > 0){
		if(FLL_Elapse_Sec_Count > 600)
	  		s_servoLocalChanAccept.b_accepted[chan] = TRUE;
	  }
	printf("===========================>>clr_phy_update_cnt[%d] = %u\n", (chan-RM_GPS_1), clr_phy_update_cnt[chan-RM_GPS_1]);
